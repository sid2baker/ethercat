...
- `(pending hash)` — recovering masters now fall back to fresh topology rediscovery when a PDO slave returns without its old fixed station address, and simulator power-cycle faults now clear fixed station identity so reconnect tests match real hardware more closely.
