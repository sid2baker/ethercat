...
- `(pending hash)` — slave reconnect recovery now retries reconnect authorization for tracked `:down` faults, so a physically restored slave does not remain stuck in `:down` if the first reconnect authorization window is missed.
