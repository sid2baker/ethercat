defmodule EtherCAT.Bus.TransactionTest do
  ...
  describe "builder functions" do
    ...
    test "apwr appends one datagram with correct cmd" do
      ...
      assert [%Datagram{cmd: 2, data: <<0x00, 0x10>>}] = Transaction.datagrams(tx)
    end

    test "aprw appends one datagram with correct cmd" do
      tx = Transaction.new() |> Transaction.aprw(0, {0x0130, <<0, 0>>})
      assert [%Datagram{cmd: 3, data: <<0, 0>>}] = Transaction.datagrams(tx)
    end

    test "armw appends one datagram with correct cmd (binary data)" do
      tx = Transaction.new() |> Transaction.armw(0, {0x0900, <<0, 0, 0, 0, 0, 0, 0, 0>>})
      assert [%Datagram{cmd: 13}] = Transaction.datagrams(tx)
    end

    test "armw appends one datagram with correct cmd (integer length)" do
      tx = Transaction.new() |> Transaction.armw(0, {0x0910, 8})
      assert [%Datagram{cmd: 13, data: <<0, 0, 0, 0, 0, 0, 0, 0>>}] = Transaction.datagrams(tx)
    end

    test "brd appends one datagram with correct cmd" do
      tx = Transaction.new() |> Transaction.brd({0x0000, 1})
      assert [%Datagram{cmd: 7, data: <<0>>}] = Transaction.datagrams(tx)
    end

    test "bwr appends one datagram with correct cmd" do
      tx = Transaction.new() |> Transaction.bwr({0x0000, <<0xFF>>})
      assert [%Datagram{cmd: 8, data: <<0xFF>>}] = Transaction.datagrams(tx)
    end

    test "brw appends one datagram with correct cmd" do
      tx = Transaction.new() |> Transaction.brw({0x0000, <<0, 0>>})
      assert [%Datagram{cmd: 9, data: <<0, 0>>}] = Transaction.datagrams(tx)
    end

    test "lrd appends one datagram with correct cmd" do
      tx = Transaction.new() |> Transaction.lrd({0x0000, 4})
      assert [%Datagram{cmd: 10, data: <<0, 0, 0, 0>>}] = Transaction.datagrams(tx)
    end

    test "lwr appends one datagram with correct cmd" do
      tx = Transaction.new() |> Transaction.lwr({0x0000, <<0xFF, 0xFF>>})
      assert [%Datagram{cmd: 11, data: <<0xFF, 0xFF>>}] = Transaction.datagrams(tx)
    end

    test "lrw appends one datagram with correct cmd" do
      tx = Transaction.new() |> Transaction.lrw({0x0000, <<0, 0, 0, 0>>})
      assert [%Datagram{cmd: 12, data: <<0, 0, 0, 0>>}] = Transaction.datagrams(tx)
    end
  end
  ...
  describe "ordering" do
    test "preserves insertion order" do
      ...
      assert [%Datagram{cmd: 4}, %Datagram{cmd: 7}, %Datagram{cmd: 12}] =
               Transaction.datagrams(tx)
    end
    ...
    test "multiple commands of the same type are distinct" do
      ...
      assert [%Datagram{cmd: 4} = d1, %Datagram{cmd: 4} = d2] = Transaction.datagrams(tx)
