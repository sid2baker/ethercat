defmodule EtherCAT.Domain.LayoutTest do
  ...
  test "register preserves PDO order and computes expected LRW WKC" do
    {0, layout} = Layout.register(Layout.new(), {:sensor, {:sm, 0}}, 2, :input)
    {2, layout} = Layout.register(layout, {:valve, {:sm, 0}}, 1, :output)
    {3, layout} = Layout.register(layout, {:valve, {:sm, 1}}, 1, :output)
    {4, layout} = Layout.register(layout, {:thermo, {:sm, 3}}, 8, :input)
  ...
  test "prepare rejects LRW images larger than one EtherCAT payload" do
    {0, layout} = Layout.register(Layout.new(), {:big, :pdo}, 2036, :output)
