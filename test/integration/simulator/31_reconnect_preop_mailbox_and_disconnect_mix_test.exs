defmodule EtherCAT.Integration.Simulator.ReconnectPreopMailboxAndDisconnectMixTest do
  ...
  test "a later counted disconnect forces recovery while a mailbox PREOP fault is still retained" do
    ...
    |> Scenario.act("mailbox reconnect PREOP rebuild retains the scripted timeout", fn _ctx ->
      Expect.eventually(
        fn ->
        ...
        attempts: 220,
        label: "mailbox reconnect PREOP rebuild retains the scripted timeout"
      )
    end)
    ...
    |> Scenario.act(
      ...
      fn %{trace: trace} ->
        Expect.eventually(
          ...
          attempts: 120,
          label: "trace captures the later disconnect inside the retained mailbox fault lifecycle"
        )
      end
    )
    |> Scenario.act(
