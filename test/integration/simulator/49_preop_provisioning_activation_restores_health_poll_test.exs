defmodule EtherCAT.Integration.Simulator.PreopProvisioningActivationRestoresHealthPollTest do
  ...
  test "provisioning activation restores runtime health polling for preop-started slaves" do
    ...
    |> Scenario.act("ring is operational and mailbox reached op", fn _ctx ->
      Expect.eventually(
        ...
        attempts: 120,
        label: "preop-first provisioning activation reaches operational"
      )
    end)
    ...
    |> Scenario.act(
      ...
      fn _ctx ->
        Expect.eventually(
          ...
          attempts: 120,
          label: "mailbox disconnect is visible after provisioning activation"
        )
      end
    )
    |> Scenario.act("mailbox reconnect heals back to op", fn _ctx ->
      Expect.eventually(
        fn ->
          Expect.master_state(:operational)
          Expect.domain(:main, cycle_health: :healthy)
        ...
        attempts: 360,
        label: "mailbox reconnect returns to op after provisioning activation"
      )
    end)
    |> Scenario.run()
  end
