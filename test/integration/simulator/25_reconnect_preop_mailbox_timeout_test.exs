defmodule EtherCAT.Integration.Simulator.ReconnectPreopMailboxTimeoutTest do
  ...
  test "reconnect-time mailbox response timeouts rerun PREOP configuration and self-heal after faults clear" do
    ...
    |> Scenario.act("mailbox drops to PREOP with retained response timeout", fn _ctx ->
      Expect.eventually(
        fn ->
        ...
        attempts: 220,
        label: "mailbox drops to PREOP with retained response timeout"
      )
    end)
    ...
    |> Scenario.act("pdo flow still works during PREOP retry window", fn _ctx ->
      Expect.eventually(
        ...
        label: "pdo flow still works during PREOP retry window"
      )
    end)
    |> Scenario.clear_faults()
    |> Scenario.act("mailbox self-heals after timeout fault clear", fn _ctx ->
      Expect.eventually(
        fn ->
          Expect.master_state(:operational)
        ...
        attempts: 220,
        label: "mailbox self-heals after timeout fault clear"
      )
    end)
    |> Scenario.run()
  end
