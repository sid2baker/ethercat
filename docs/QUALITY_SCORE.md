...
| Hardware Harness | `B+` | Livebook and loopback benchmarks exist; further hardware-specific checks should keep accumulating here. |
...
3. Add more harness checks that can be run unattended from notebooks or scripts.
4. Keep the docs indices and active plans current via `mix ethercat.harness.doctor`.
